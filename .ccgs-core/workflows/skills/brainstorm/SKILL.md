...
2. **Check for existing concept work**:
   - Read `CCGS-Data/design/gdd/game-concept.md` if it exists (resume, don't restart)
   - Read `CCGS-Data/design/gdd/game-pillars.md` if it exists (build on established pillars)
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **在关键决策点使用 Markdown 编号列表** throughout brainstorming:
...
Present all three. Then 向用户呈现选项以 capture the selection.
...
将这些作为编号列表选项呈现 — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, 向用户呈现选项（使用 Markdown 编号列表）:
...
If the user selects B, C, or D, make the revision, 然后再次向用户呈现选项:
...
- **`creative-director`** — gate **CD-PILLARS** (`.ccgs-core/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`.ccgs-core/docs/director-gates.md`)
...
Collect both verdicts, then present them together 使用编号列表:
...
- **Target platform**: 向用户呈现选项 — "What platforms are you targeting for this game?"
...
- **Engine experience**: 向用户呈现选项 — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`.ccgs-core/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`.ccgs-core/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `.ccgs-core/docs/templates/game-concept.md`. Fill in ALL sections from the
...
5. 向用户呈现选项以 write approval:
- Prompt: "Game concept is ready. May I write it to `CCGS-Data/design/gdd/game-concept.md`?"
- Options: `[A] Yes — write it` / `[B] Not yet — revise a section first`

If [B]: ask which section to revise 使用 Markdown 编号列表交互 with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then 向用户呈现选项 — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `.ccgs-core/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
6. **Suggest next steps** (in this order — this is the professional studio
   ...
   3. "Use `/design-review CCGS-Data/design/gdd/game-concept.md` to validate concept completeness before going downstream"
...
> to `CCGS-Data/design/gdd/game-concept.md`. Open a fresh Claude Code session to continue
