...
### Manual verification with confirmation (use Markdown 编号列表)
...
Batch up to 4 manual verification questions into a single Markdown 编号列表 call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified via Markdown 编号列表
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via Task using gate **QL-TEST-COVERAGE** (`.ccgs-core/docs/director-gates.md`).
...
Spawn `lead-programmer` via Task using gate **LP-CODE-REVIEW** (`.ccgs-core/docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via Markdown 编号列表:
...
- 向用户呈现选项以 the code review prompt and for batching manual
