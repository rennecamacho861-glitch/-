...
2. **Questions**: Ask clarifying questions specific to this section. Use
   Markdown 编号列表 for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then 向用户呈现选项以 capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   Markdown 编号列表. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use Markdown 编号列表 with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: 向用户呈现选项（使用 Markdown 编号列表）:
...
- Surface any disagreements between agents to the user via Markdown 编号列表
...
- Present the specialists' proposals to the user for review via Markdown 编号列表
...
- The main session presents the agent's output to the user via Markdown 编号列表
- The user decides; the main session writes to file
