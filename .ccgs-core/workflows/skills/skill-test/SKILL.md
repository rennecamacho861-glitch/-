...
Validates `.ccgs-core/workflows/skills/*/SKILL.md` files for structural compliance and
...
- `static all` → run 7 structural checks on all skills (Glob `.ccgs-core/workflows/skills/*/SKILL.md`)
- `spec [name]` → read skill + test spec, evaluate assertions
- `category [name]` → run category-specific rubric from `.ccgs-core/tests/quality-rubric.md`
...
Find skill at `.ccgs-core/workflows/skills/[name]/SKILL.md`.
Look up the spec path from `.ccgs-core/tests/catalog.yaml` — use the
`spec:` field for the matching skill entry.

If either is missing:
- Missing skill: "Skill '[name]' not found in `.ccgs-core/workflows/skills/`."
...
Date: [date]
Spec: .ccgs-core/tests/skills/[category]/[name].md
...
"May I write these results to `.ccgs-core/tests/results/skill-test-spec-[name]-[date].md`
and update `.ccgs-core/tests/catalog.yaml`?"

If yes:
- Write results file to `.ccgs-core/tests/results/`
- Update the skill's entry in `.ccgs-core/tests/catalog.yaml`:
  - `last_spec: [date]`
...
Find skill at `.ccgs-core/workflows/skills/[name]/SKILL.md`.
Look up `category:` field in `.ccgs-core/tests/catalog.yaml`.
...
Read `.ccgs-core/tests/quality-rubric.md`.
...
"May I update `.ccgs-core/tests/catalog.yaml` to record this category check
...
Read `.ccgs-core/tests/catalog.yaml`. If missing, note that catalog doesn't exist
yet (first-run state).

### Step 2 — Enumerate All Skills and Agents

Glob `.ccgs-core/workflows/skills/*/SKILL.md` to get the complete list of skills.
Extract skill name from each path (directory name).

Also read the `agents:` section from `.ccgs-core/tests/catalog.yaml` to get the
complete list of agents.

### Step 3 — Build Skill Coverage Table

For each skill:
- Check if a spec file exists (use the `spec:` path from catalog, or glob `.ccgs-core/tests/skills/*/[name].md`)
...
- Check if a spec file exists (use the `spec:` path from catalog, or glob `.ccgs-core/tests/agents/*/[name].md`)
...
- After `spec [name]` PASS: "Update `.ccgs-core/tests/catalog.yaml` to record this
...
- After `audit`: "Start with the critical-priority gaps. Use the spec template
  at `.ccgs-core/tests/templates/skill-test-spec.md` to create new specs."
