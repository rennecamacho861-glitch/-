...
a **confirm/adjust** prompt 使用 Markdown 编号列表交互 — not open-ended questions.
...
decisions remain, present each one as a separate Markdown 编号列表 with the proposed
...
4.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` via Task using gate **TD-ADR** (`.ccgs-core/docs/director-gates.md`):
...
5. **Write approval** — 向用户呈现选项（使用 Markdown 编号列表）:
...
After the ADR is written (and registry optionally updated), close with Markdown 编号列表.
