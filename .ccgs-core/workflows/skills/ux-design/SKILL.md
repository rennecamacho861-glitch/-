...
Read `.ccgs-core/docs/technical-preferences.md` and extract the `## Input & Platform`
...
2. **Questions**: Ask what is needed to draft this section. 向用户呈现选项（使用 Markdown 编号列表）
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then 向用户呈现选项以 capture the decision.
...
- 向用户呈现选项（使用 Markdown 编号列表）: "Want an ASCII wireframe as part of this spec?"
...
向用户呈现选项以 surface any open questions on accessibility tier:
...
向用户呈现选项以 step through items in groups of 3-4, not all at once.
