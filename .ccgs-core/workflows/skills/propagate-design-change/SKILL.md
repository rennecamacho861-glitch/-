...
**Usage:** `/propagate-design-change CCGS-Data/design/gdd/combat-system.md`
...
> "Usage: `/propagate-design-change CCGS-Data/design/gdd/[system].md`
...
Markdown 编号列表`bash
git show HEAD:CCGS-Data/design/gdd/[filename].md
Markdown 编号列表`
...
Read all ADRs in `CCGS-Data/project-docs/architecture/`:
...
Read `CCGS-Data/project-docs/architecture/architecture-traceability.md` if it exists.
...
Spawn `technical-director` via Task using gate **TD-CHANGE-IMPACT** (`.ccgs-core/docs/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use Markdown 编号列表 with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
...
If `CCGS-Data/project-docs/architecture/architecture-traceability.md` exists:
...
Ask: "May I write the change impact report to `CCGS-Data/project-docs/architecture/change-impact-[date]-[system-slug].md`?"
