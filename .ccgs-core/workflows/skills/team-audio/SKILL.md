...
> Usage: `/team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). 不使用选择题交互; 直接输出 the guidance directly.
...
Spawn the **primary engine specialist** in parallel (from `.ccgs-core/docs/technical-preferences.md` Engine Specialists) to validate the integration approach:
