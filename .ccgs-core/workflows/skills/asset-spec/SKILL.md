...
- If it exists: read it, find the first context (system/level/character) with any asset at status "Needed" but no spec file written yet, and 向用户呈现选项（使用 Markdown 编号列表）:
...
Present the full identified list to the user. 向用户呈现选项（使用 Markdown 编号列表）:
...
After presenting all specs, 向用户呈现选项（使用 Markdown 编号列表）:
