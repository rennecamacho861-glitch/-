...
If the project appears fresh (no artifacts at all), 向用户呈现选项（使用 Markdown 编号列表）:
...
Read `.ccgs-core/docs/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
and offer to handle it immediately 使用 Markdown 编号列表交互. Choose the first
branch that applies:
