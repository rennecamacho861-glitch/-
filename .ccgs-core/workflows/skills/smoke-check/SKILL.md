...
3. **Engine detection**: read `.ccgs-core/docs/technical-preferences.md` and
...
"Engine not configured in `.ccgs-core/docs/technical-preferences.md`. Run
...
2. The current sprint plan from `CCGS-Data/production/sprints/` (most recently modified
   file)
...
1. Extract the system slug from the story's file path
   (e.g., `CCGS-Data/production/epics/combat/story-001.md` → `combat`)
...
向用户呈现选项以 batch-verify. Keep to at most 3 calls.
...
- 向用户呈现选项以 all manual smoke check verification.
