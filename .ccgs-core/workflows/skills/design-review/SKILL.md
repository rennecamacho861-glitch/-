...
| Engine-specific patterns or APIs | Primary engine specialist (from `.ccgs-core/docs/technical-preferences.md`) |
...
向用户呈现选项以 ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab Markdown 编号列表 before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and 向用户呈现选项以 a **post-revision closing widget**:
