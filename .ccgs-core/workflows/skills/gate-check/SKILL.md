...
Before generating the final verdict, spawn all four directors as **parallel subagents** via Task using the parallel gate protocol from `.ccgs-core/docs/director-gates.md`. Issue all four Task calls simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`.ccgs-core/docs/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`.ccgs-core/docs/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`.ccgs-core/docs/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`.ccgs-core/docs/director-gates.md`)
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt 使用 Markdown 编号列表交互.
...
- **No accessibility requirements doc?** → 向用户呈现选项以 offer to create it now:
  ...
  - If "Create it now": 向用户呈现选项以询问 for the tier:
  ...
  - Then write `design/accessibility-requirements.md` using the template at `.ccgs-core/docs/templates/accessibility-requirements.md`, filling in the chosen tier. Confirm: "May I write `design/accessibility-requirements.md`?"
