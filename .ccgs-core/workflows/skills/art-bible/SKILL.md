...
Read `CCGS-Data/design/gdd/game-concept.md`. If it does not exist, fail with:
...
**Retrofit mode detection**: Glob `CCGS-Data/design/art/art-bible.md`. If the file exists:
- Read it in full
...
- Present this table to the user:
  > "Found existing art bible at `CCGS-Data/design/art/art-bible.md`. [N] sections are complete, [M] need content. I'll work on the incomplete sections only — existing content will not be touched."
...
Read `.ccgs-core/docs/technical-preferences.md` if it exists — extract performance budgets and engine for asset standard constraints.
...
Use Markdown 编号列表 with two tabs:
...
Present the art-director's draft to the user. 向用户呈现选项（使用 Markdown 编号列表）:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — 向用户呈现选项以 let the user decide.
...
- **`technical-artist`**: Engine-specific hard constraints — poly count budgets per asset category, texture memory limits, material slot counts, importer constraints, anything from the performance budgets in `.ccgs-core/docs/technical-preferences.md`
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via Task using gate **AD-ART-BIBLE** (`.ccgs-core/docs/director-gates.md`).
...
- Does `CCGS-Data/design/gdd/systems-index.md` exist? → map-systems is done, skip that option
- Does `.ccgs-core/docs/technical-preferences.md` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
- Does `CCGS-Data/design/gdd/` contain any `*.md` files? → design-system has been run, skip that option
- Does `CCGS-Data/design/gdd/gdd-cross-review-*.md` exist? → review-all-gdds is done
- Do GDDs exist (check above)? → include /consistency-check option

向用户呈现选项以 next steps. Only include options that are genuinely next based on the state check above:
