...
When presenting decision options to the user, use a standard Markdown numbered list.
Follow the **Explain -> Capture** pattern:

1. **Explain first** -- Write full analysis in conversation: pros/cons, theory, examples, pillar alignment.
2. **Present Options** -- Use a numbered list, with a one-sentence description of the trade-offs for each option. User picks or types a custom answer.
...
- If running as a Task subagent, structure text clearly so the orchestrator can present it.
  Wait for the user's numbered reply before continuing.
