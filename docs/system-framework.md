...
版本：v0.8.6，对应规则书 v0.8.6 情报真值、开局装备与敌人生态反馈  
用途：把规则拆成可直接实现的数据、流程、模块和待确认问题。本文不替代规则书；规则书决定玩法，本文帮助代码落地。
...
- `combatSystem`：照面创建、动作选择、先后手、伤害、重伤、优势窗口。
...
- v0.6.7 起删除全局危险数值和危险格；手枪、报警、说服失败等只保留直接效果、日志、情报和位置提示。
...
来源修正默认：首次看见 +1；视野领先 +2；防御承受动作 +3；成功躲闪直接至少 1 点且修正 +2；对方使用道具 +3；陷阱直接给 1 条；说服成功直接给 1-2 条。所有来源都只能产出 `statExact` 或 `itemExact`。
...
实现时不要显示固定 `x/4` 或“压力 P”，只显示“轮次 N”和当前优势状态。
## 8. 道具落地框架
...
- 手枪：远程攻击，5 次，视野内 4 格，命中 3 伤害；不修改全局危险；真实开火后向 `feedbackEvents` 推送 `gunshot`，由 Phaser 画枪口火光和弹道线。
...
- 眼镜：躲闪成功后揭示对方 1 件道具。
- 照明棒：短时扩大视野。
- 回声针：橙色 rare；不再输出方向/距离文字，改为向 `feedbackEvents` 推送 `echo-pulse`，并把最近未清空道具节点或 AI 所在的 `2x2` 四格回响区写入 `MapState.hints` 与反馈事件 `positions`。
- 优势联动战斗道具：肋钩、绊踝线、追步刺、反压铁片、压胆钉、定神线。玩家只在优势窗口使用；敌人可在敌方优势窗口或 `enemyBonus` 存在时使用；效果统一写入 `EncounterState.activeEffects`。
- 掉落：敌人被击败时，若存在可掉落装备则保底 1 件，其余装备按 35% 独立检定补充，最多 2 件，5% 概率额外 1 件，并保留 charges/durability/usedFlags；若无候选，生成“未发现可回收道具”反馈。
...
- `advantagePress`：敌方优势窗口后，进攻、远程、压制道具权重上升。
...
1. 玩家初始属性固定为 3/3/3/3/3，并在新局开始时通过 `pendingPickupOffer.nodeId = "starter"` 选择回声针、手枪、绷带之一。
...
- `SimulationCommandPort`：供 HUD/输入层调用 `move`、`choosePickup`、`useItem`、`playCombatAction`、`tryFlee`、`continueFight`、`tryPersuade`、`reset`。
...
2. 若继续增加战斗动作与伤害分支，先拆 `combatRoundSystem`，把 `resolveActionRound`、近战/远程伤害和优势窗口统一迁出。
...
- 敌人道具情报使用 `IntelEntry.itemId` 绑定 `ITEMS[itemId]`；HUD 悬浮主说明必须通过 `itemUiDescription(itemId)` 取得玩家可读介绍，反制说明继续读取 `counterplay`。
...
- `ItemDefinition.description` 可以保留规则摘要和实现校验信息；玩家可见的拾取卡片、背包 title、情报 hover、状态条 title 与拾取日志统一通过 `src/sim/itemText.ts` 的 `itemUiDescription()` 取得，不得直接照搬端口、事件或动画实现描述。
...
- 手枪为 `rechargeable`，0 弹保留在背包，可由旧弹夹补充。
