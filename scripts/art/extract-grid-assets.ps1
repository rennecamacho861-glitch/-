param(
  ...
  [string]$SourceImagePath = "CCGS-Data/design/art/source/grid-dungeon/grid-dungeon-components-iter01.png",
...
if (!(Test-Path -LiteralPath $manifestFullPath)) { throw "Manifest not found: $manifestFullPath" }
...
try {
  foreach ($asset in $manifest.assets) {
    $assetSourcePath = if ($asset.PSObject.Properties.Name -contains "sourceImage") {
      [string]$asset.sourceImage
    } else {
      $SourceImagePath
    }
    $source = Get-SourceBitmap $assetSourcePath $sourceCache
    ...
    Remove-ChromaGreen $cropped $GreenTolerance

    $outWidth = [int]$asset.outputSize.w
    $outHeight = [int]$asset.outputSize.h
    $final = if ($cropped.Width -eq $outWidth -and $cropped.Height -eq $outHeight) {
      $cropped
    } else {
      $resized = Resize-Bitmap $cropped $outWidth $outHeight
      $cropped.Dispose()
      $resized
    }
...
"Extracted $($written.Count) transparent assets."
